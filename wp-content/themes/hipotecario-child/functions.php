...
function gi_getcv_module() {
  ...
  $pageid = trim(strip_tags($_POST["pageid"]));

  $filegeneratedname = time()."_".generateRandomString();
  $extension = end(explode(".",  $_FILES['file']["name"]));
  $fullgeneratedfilename = $filegeneratedname.".".$extension;

  $correo_a = get_field("correo_a", $pageid);
  $correo_b = get_field("correo_b", $pageid);
  $correo_c = get_field("correo_c", $pageid);
  $copyemails = "";
  ...
  if( ($extension=="pdf") || ($extension=="doc") || ($extension=="docx") )
  {
    ...
    if ( ( copy($_FILES["file"]["tmp_name"], $to . $filegeneratedname.".".$extension ) ) ):
      ...
      $titlesubj ="Bolsa de Trabajo - ".$sitename;
      ...
      $cabeceras2 .= 'To: '.$correo_a. "\r\n";
      $cabeceras2 .= 'From: Contacto web <no-reply@bancohipotecario.com.sv>' . "\r\n";
      if($correo_a!="")
      {
        $copyemails .= $correo_a;
      }
      ...
      $body .= "<strong>Interés A:</strong>  ".trim(strip_tags($_POST["intereses1"]))."<br>";
      $body .= "<strong>Interés B:</strong>  ".trim(strip_tags($_POST["intereses2"]))."<br>";
      $body .= "<strong>Interés C:</strong>  ".trim(strip_tags($_POST["intereses3"]))."<br>";
      $body .= "<strong>Fecha:</strong>  ".date("Y-m-d h:i:s")."<br>";
      $body .= "<a href='".$urlsite."/wp-content/themes/hipotecario-child/uploads/cv/".$fullgeneratedfilename."' target='_blank'>Ver documento adjunto</a>";

      mail($correo_a, $titlesubj, $body, $cabeceras2);
...
function gi_receiveformcontacto_module() {
  ...
  $consulta = trim(strip_tags($_POST["consulta"]));
	$pageid = trim(strip_tags($_POST["pageid"]));
	$emailtosend = get_field("correo_receptor", $pageid );
    ...
    $titlesubj ="Contacto desde sitio web ".$sitename;

    $bodyu = "<strong>Tipo de Consulta:</strong>  ".$tipoconsulta."<br><br>";
    $bodyu .= "<strong>Nombre completo:</strong>  ".$nombrecompleto."<br>";
    $bodyu .= "<strong>Correo electrónico:</strong>  ".$correoelectronico."<br>";
    $bodyu .= "<strong>Teléfono:</strong>  ".$telefono."<br>";
    $bodyu .= "<strong>Departamento:</strong>  ".$departamento."<br>";

    $bodyu .= "<strong>Consulta/ Comentarios:</strong><br>";
    $bodyu .= "".$consulta."<br>";
    ...
    // Cabeceras adicionales
    $cabeceras2 .= 'To: '.$emailtosend. "\r\n";
    $cabeceras2 .= 'From: Contacto web <no-reply@bancohipotecario.com.sv>' . "\r\n";

    mail($emailtosend, $titlesubj, $bodyu, $cabeceras2);
  ...
  */
  ...
  echo json_encode(array("status"=>1, "type"=>"success", "title"=>"Contacto", "message"=>"Gracias por escribirnos, hemos recibido tus comentarios. Pronto nos pondremos en contacto. ".$emailtosend));
