<?php
/**
 * Modify WooCommerce features according to yttor uses.
 *
...
function stag_woocommerce_script_styles() {
	...
	wp_enqueue_style( 'yttor-woocommerce', get_template_directory_uri() . '/woocommerce/woocommerce.css', '', STAG_THEME_VERSION, 'all' );
	wp_enqueue_script( 'yttor-woocommerce', get_template_directory_uri() . '/woocommerce/woocommerce.js', array( 'jquery' ), STAG_THEME_VERSION, true );
...
function yttor_single_product_page_navigation(){
	?>
...
add_action( 'woocommerce_before_single_product', 'yttor_single_product_page_navigation', 2 );
...
add_action( 'yttor_middle_header', 'stag_woocommerce_cart_dropdown' );
 ...
 */
function yttor_default_tab_description_title() {
	return __( 'Product Description', 'stag' );
}
add_filter( 'woocommerce_product_description_tab_title', 'yttor_default_tab_description_title' );
add_filter( 'woocommerce_product_description_heading', '__return_false' );

function yttor_default_tab_information_title() {
	return __( 'Additional Details', 'stag' );
}
add_filter( 'woocommerce_product_additional_information_tab_title', 'yttor_default_tab_information_title' );
add_filter( 'woocommerce_product_additional_information_heading', '__return_false' );

function yttor_default_tab_reviews_title() {
	return __( 'Customer Reviews', 'stag' );
}
add_filter( 'woocommerce_product_reviews_tab_title', 'yttor_default_tab_reviews_title' );

function yttor_change_order_button_text() {
	return __( 'Proceed to Checkout', 'stag' );
}
add_filter( 'woocommerce_order_button_text', 'yttor_change_order_button_text' );
...
function stag_woocommerce_add_category_fields() {
	...
	<div class="form-field">
		...
		<select id="display_sidebar" name="display_sidebar" class="postform">
			<option value=""><?php _e( 'Default - Set in yttor > Sidebar', 'stag' ); ?></option>
...
function stag_woocommerce_edit_category_fields( $term, $taxonomy ) {
	...
	<tr class="form-field">
		...
		<td>
			<select id="display_sidebar" name="display_sidebar" class="postform">
				<option value="" <?php selected( '', $display_sidebar ); ?>><?php _e( 'Default - Set in yttor > Sidebar', 'stag' ); ?></option>
 ...
 * @since 1.2
 */
function yttor_is_older_than_2_1() {
	global $woocommerce;
...
if ( ! yttor_is_older_than_2_1() ) :
	...
	function yttor_before_shipping_title() {
		echo '<h3 id="payment_method_heading">' . __( 'Payment Method', 'stag' ) . '</h3>';
	}
	add_action( 'woocommerce_review_order_before_payment', 'yttor_before_shipping_title' );

	remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_rating', 10 );

endif; // yttor_is_older_than_2_1 check
